    ...
    int minStones(vector<int> p, vector<int> w) {
        ...
        for(int i=n-1;i>=0;i--){
            if(children[i].empty()){
                ...
                sort(all(children[i]), [&](int j, int k){
                    return ans[j]-w[j] > ans[k]-w[k];
                });

                int tmp = 0, tmpans = 0, children_w = 0;
                for(int child: children[i]){
                    tmpans = max(tmpans, tmp + ans[child]);
                    tmp += w[child];
                    children_w += w[child];
                }
                ans[i] = max(tmpans, w[i] + children_w);
            }
        }
