...
class FiveRooks {
    ...
    vector<int> find(vector<string> board) {
        vector<int> cnti(8, 0), cntj(8, 0);
        rep(8) repp(j, 8) if(board[i][j] == 'R'){
            cnti[i]++;
            cntj[j]++;
        }
        ...
        rep(8) repp(j, 8) if(board[i][j] == 'R'){
            que.push(make_pair(-cnti[i]-cntj[j], Pii(i, j)));
        }
        ...
        while(cnt < 5){
            ...
            if(cnti[p.first] != -1 && cntj[p.second] != -1){
