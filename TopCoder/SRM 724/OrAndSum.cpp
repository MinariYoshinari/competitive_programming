...
class OrAndSum {
    ...
    string isPossible(vector<long long> pairOr, vector<long long> pairSum) {
        ...
        rep(n){
            ...
            pairAnd[i] = (ll)(pairSum[i] - pairOr[i]);
        }
        ...
        repp(j, 60){
            int flag = 1;
            for(ll b=0LL;b<=1LL;b++){
                flag = 1;
                array[0] |= (b << j);
                reppp(i, 1, n+1){
                    if(pairAnd[i-1] & (1LL << j)){
                        array[i] |= (1LL << j);
                    }else if((n > 1) && (i != n) && (pairAnd[i] & (1LL << j))){
                        array[i] |= (1LL << j);
                    }else if(
                        (pairOr[i-1] & (1LL << j)) && 
                        !(array[i-1] & (1LL << j))
                    ...
                    if((pairAnd[i-1] & (1LL << j)) ^ ((array[i-1] & (1LL << j)) & (array[i] & (1LL << j)))){
                        flag = 0;
                        break;
                    }
                    if(( pairOr[i-1] & (1LL << j)) ^ ((array[i-1] & (1LL << j)) | (array[i] & (1LL << j)))){
                        flag = 0;
                        break;
                    }
                }
        ...
        rep(n){
            if(array[i] + array[i+1] != pairSum[i]){
                return "Impossible";
            }
        }
        return "Possible";
