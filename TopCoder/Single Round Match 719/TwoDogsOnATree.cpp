...
class TwoDogsOnATree {
    ...
    void dfs(vector<int> graph[], vector<int> w, int node, int value){
        a.pb(value);
        for(int next: graph[node]){
            dfs(graph, next, value^w[next]);
        }
        return;
    }

    int maximalXorSum(vector<int> parent, vector<int> w) {
        vector<int> graph[(int)parent.size() + 1];
        rep((int)parent.size()){
            graph[parent[i]].pb(i+1);
        }
